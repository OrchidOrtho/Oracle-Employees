...
SELECT  DISTINCT    OE.[Employee first name] AS firstname, OE.[Employee Last Name] AS lastname, OE.[Employee middle name initial] AS initial, OE.[Employee e-mail] AS email, 
		   ...
		   CASE OE.[Employment type] WHEN 'ACTIVE' THEN 'A' ELSE 'I' END AS p_active, 
           ' ' AS dept, OE.[Seniority date] AS SeniorityDate, s.HAL_SiteID, OE.Location,
		   COALESCE(oe.[Business Title], 'MISSING') as BusinessTitle,  COALESCE(o.BusinessUnitName, 'MISSING') as BusinessUnitName,   COALESCE(O.DeptName, 'MISSING') as  DeptName
FROM         dbo.OracleEE as OE LEFT OUTER JOIN dbo.OOSAEL as o ON
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'_vwMQ1Test';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'_vwMQ1Test';
